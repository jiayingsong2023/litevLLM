...
**修复后典型结果**（`Qwen3.5-9B-GGUF` Q4_K_M vs `Qwen3.5-9B-FP16`；回归套件默认 `FASTINFERENCE_KV_FP8=1` 以省显存，严格对拍可设 `FASTINFERENCE_KV_FP8=0`）：`verify_semantic_integrity --preset qwen35_9b_gguf` 的 Prefill **CosSim≈0.99**，greedy 首 token 与 HF 一致；`quality_bar_spotcheck` 在贪婪解码下可读性/连贯性可通过。
...
- **Paged prefill**：`tests/test_qwen35_paged_prefill_vs_torch_reference.py`（默认 `FASTINFERENCE_KV_FP8=1`）。
