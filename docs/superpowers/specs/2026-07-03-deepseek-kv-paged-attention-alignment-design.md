...
`DeepSeekV4FlashForCausalLM` 按 `(context_length, head_dim, dtype, device)` 缓存共享 pool。每个 request state 只绑定一个 request view，不再拥有一份独立 dense cache。
...
| allocator | 每组 `_LazyPagedRows` 内部一个 `BlockAllocator` | 复用现有安全检查；不保留旧 `DeepSeekV4KVPageAllocator`。 |
...
这是 grow-only pool：已创建的 backing chunks 不释放给系统 allocator，避免并发请求之间反复分配大 tensor；请求释放的是 block ownership 和 per-request metadata。
...
- request views 跨请求共享同一个 model-level pool。
