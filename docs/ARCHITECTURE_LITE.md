...
- **统一配置构建**: offline 与 server 共用 `config_builder.py`，先通过 `RuntimeProfileRegistry` 解析 profile data，再生成 `VllmConfig + RuntimeConfig`。production code 以 profile/runtime policy 为策略真源，不再把环境变量当作模型热路径配置入口。
...
- **Production path**: profile data -> `RuntimeConfig` -> `attn_metadata["config"]` / runtime policy objects。
- **Benchmark / tuning path**: tools 可以把临时 tuning snapshot 安装到运行时，用于 kernel 探索、profile 试验或诊断，但这不是 production runtime 的策略真源。
...
其中 26B 默认使用 benchmark / latency profiles 中的 MoE int4 decode strategy，31B 使用 dense Gemma4 recommended adapter profile，并且两者都已经完全接入阶段 3 高并发二维向量化 BatchSampler 采样优化。更激进的 kernel 组合仍属于 benchmark-tool 调参场，不是常规 production runtime switches。
