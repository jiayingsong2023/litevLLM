...
def benchmark_fused_vs_unfused():
    ...
    # 1. 准备输入数据
    q = torch.randn(
        (num_tokens, num_heads, head_dim), device="cuda", dtype=torch.float16
    )
    k = torch.randn(
        (num_tokens, num_kv_heads, head_dim), device="cuda", dtype=torch.float16
    )
    v = torch.randn(
        (num_tokens, num_kv_heads, head_dim), device="cuda", dtype=torch.float16
    )
    ...
    def run_fused():
        fused_prefill_attention(
            q, k, v, k_cache, v_cache, slot_mapping, out_fused, 0.125
        )
