...
def test_resolve_model_cls_supports_qwen2_vl_official_architecture() -> None:
    ...
    assert issubclass(model_cls, nn.Module)
    assert model_cls.__name__ == "Qwen2VLForConditionalGeneration"
    assert arch == "Qwen2VLForConditionalGeneration"


def test_resolve_model_cls_raises_for_empty_architectures_without_inference() -> None:
    reg = _ModelRegistry()
    model_cfg = SimpleNamespace(
        hf_config=SimpleNamespace(model_type="", text_config=None)
    )
