...
def _decode_generated_text(
    tokenizer: Any, token_ids: list[int], sampling_params: Any
) -> str:
...
class OutputProcessor:
    ...
    def needs_partial_text_for_early_stop(self) -> bool:
        """Return True if finalize_step must decode partial text each step.

        The base implementation never early-stops, so no per-step decode is
        required. Subclasses that inspect partial text in ``should_early_stop``
