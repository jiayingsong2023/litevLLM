...
def rmsnorm_awq_fused_kernel(
    ...
    for k in range(0, K, BLOCK_K):
        ...
        a_row = tl.load(row_a_ptr + k_offs[None, :] * stride_ak, mask=k_offs[None, :] < K, other=0.0).to(tl.float32)
    ...
    b_row_ptr = b_ptr + offs_bn[:, None] * stride_bn
    s_row_ptr = s_ptr + offs_bn[:, None] * stride_sn
    z_row_ptr = z_ptr + offs_bn[:, None] * stride_zn
    ...
    for k in range(0, K, BLOCK_K):
        ...
        b_packed = tl.load(b_row_ptr + (current_k_offs[None, :] // 8) * stride_bk,
                           mask=(offs_bn[:, None] < N) & (current_k_offs[None, :] < K), other=0)
        
        g_idx = current_k_offs // group_size
        scales = tl.load(s_row_ptr + g_idx[None, :] * stride_sk, 
                         mask=(offs_bn[:, None] < N) & (g_idx[None, :] < (K // group_size)), other=1.0)
        z_packed = tl.load(z_row_ptr + (g_idx[None, :] // 8) * stride_zk,
                           mask=(offs_bn[:, None] < N) & (g_idx[None, :] < (K // group_size)), other=0)
        
        shift = (current_k_offs[None, :] % 8) * 4
        zeros = (z_packed >> shift) & 0xF
        b = ((b_packed >> shift) & 0xF).to(tl.float32)
        b = (b - zeros.to(tl.float32)) * scales.to(tl.float32)
        
        accumulator += tl.dot(a_normed.to(tl.float16), tl.trans(b.to(tl.float16)))

    tl.store(c_ptr + stride_cm * offs_am[:, None] + stride_cn * offs_bn[None, :], 
             accumulator.to(tl.float16), mask=(offs_am[:, None] < M) & (offs_bn[None, :] < N))
