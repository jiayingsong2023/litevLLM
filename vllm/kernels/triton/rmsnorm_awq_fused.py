...
def rmsnorm_awq_fused_kernel(
    ...
    for k in range(0, K, BLOCK_K):
        ...
        mask = k_offs < K
        a_row = tl.load(
            row_a_ptr + k_offs[None, :] * stride_ak, mask=mask[None, :], other=0.0
        ).to(tl.float32)
    ...
    for k in range(0, K, BLOCK_K):
        ...
        a = tl.load(
            row_a_ptr + current_k_offs[None, :] * stride_ak,
            mask=mask_k[None, :],
            other=0.0,
        ).to(tl.float32)
        ...
        b_packed = tl.load(
            b_base_ptr + (current_k_offs[None, :] // 8) * stride_bk,
            mask=(offs_bn[:, None] < N) & (mask_k[None, :]),
            other=0,
        )
        ...
        scales = tl.load(
            s_base_ptr + g_idx[None, :] * stride_sk,
            mask=(offs_bn[:, None] < N) & (mask_k[None, :]),
            other=1.0,
        )
        z_packed = tl.load(
            z_base_ptr + (g_idx[None, :] // 8) * stride_zk,
            mask=(offs_bn[:, None] < N) & (mask_k[None, :]),
            other=0,
        )
...
def rmsnorm_awq_fused_linear(x, qweight, scales, qzeros, group_size, norm_w, eps=1e-6):
    ...
    rmsnorm_awq_fused_kernel[grid](
        ...
        stride_zn=qzeros.stride(0),
        stride_zk=qzeros.stride(1),
