...
def _q8_0_raw_gate_up_activation_kernel(
    ...
    for block_idx in tl.static_range(0, BLOCKS_PER_ROW):
        ...
        gate_values = (
            tl.load(gate_raw_ptr + block_offset + 2 + offsets)
            .to(tl.int8)
            .to(tl.float32)
        )
        ...
        up_values = (
            tl.load(up_raw_ptr + block_offset + 2 + offsets).to(tl.int8).to(tl.float32)
        )
...
def q8_0_raw_linear(
    ...
    *,
    ...
    if raw_payload.ndim != 1:
        raise ValueError(f"raw Q8_0 payload must be 1-D; got {raw_payload.ndim}-D")
