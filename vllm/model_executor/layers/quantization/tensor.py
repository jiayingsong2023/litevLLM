...
def dequantize_symmetric_packed_int4_pytorch(
    ...
    in [0, 15]; map to signed with (q - 8) * scale per group.
    """
    try:
        if qweight.dim() < 2:
            raise RuntimeError(
                f"qweight must be at least 2D, got {tuple(qweight.shape)}"
            )
        leading_shape = tuple(qweight.shape[:-2])
        ...
        if n_cols % group_size != 0:
            raise RuntimeError(
                f"n_cols={n_cols} not divisible by group_size={group_size}"
            )
        n_groups = n_cols // group_size
        qs = qs.reshape(*leading_shape, n_rows, n_groups, group_size)
