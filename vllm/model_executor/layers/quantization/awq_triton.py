...
def awq_dequantize_kernel(
    ...
    ptr_q = (
        qweight_ptr + offs_k[:, None] * stride_qw + (offs_n[None, :] // 8) * stride_qn
    )
    ...
    tl.store(
        ...
        (offs_k[:, None] < n_rows) & (offs_n[None, :] < n_cols),
    )


def awq_dequantize_triton(
    qweight, scales, zeros, group_size=128, out_dtype=torch.float16
):
    # Determine Layout
    # qweight: [R, C/8]
    # scales: [SR, SC]
    ...
    grid = lambda META: (
        ...
        triton.cdiv(n_rows, META["BLOCK_K"]),
    )
