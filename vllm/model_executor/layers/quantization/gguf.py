...
def _fused_moe_gguf(
    ...
    from .gguf_kernels import ggml_dequantize_fallback
    import gguf

    # 1. Get dequantized weights (cached)
    # For MoE, w1/w2 are [E, rows, cols_bytes]
    # We need to calculate the actual rows (m) and cols (n) for the experts
    bs1, ts1 = gguf.GGML_QUANT_SIZES[qweight_type]
    n1 = w1.shape[2] // ts1 * bs1
    m1 = w1.shape[1]
    
    bs2, ts2 = gguf.GGML_QUANT_SIZES[qweight_type2]
    n2 = w2.shape[2] // ts2 * bs2
    m2 = w2.shape[1]

    # Dequantize all experts in this layer at once (or get from cache)
    w1_dequant = ggml_dequantize_fallback(w1, qweight_type, m1, n1, x.dtype)
    w2_dequant = ggml_dequantize_fallback(w2, qweight_type2, m2, n2, x.dtype)

    # 2. Call optimized Fused MoE kernel
