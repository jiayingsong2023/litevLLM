...
def ggml_dequantize_fallback(
    W: torch.Tensor, quant_type: int, m: int, n: int, dtype: torch.dtype = torch.float16
) -> torch.Tensor:
    ...
    if res_flat.numel() != m * n:
        ...
        copy_n = min(
            n, res_flat.shape[1] if res_flat.dim() > 1 else res_flat.numel() // copy_m
        )
    ...
    if len(_DEQUANT_CACHE) > _CACHE_CAPACITY:
        _DEQUANT_CACHE.popitem(last=False)
    return res
...
def ggml_mul_mat_vec_a8_fallback(
    ...
    working_weight = (
        weight.to(X.dtype) if weight.dtype == torch.float8_e4m3fn else weight
    )
