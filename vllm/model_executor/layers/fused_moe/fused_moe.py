...
class FP8ExpertCache:
    ...
    def get(self, expert_id, w1_orig, w2_orig):
        ...
        if w2_orig is not None:
            w1_f8, w1_s = _convert_to_fp8_with_scales(
                w1_orig[expert_id]
                if isinstance(expert_id, (int, torch.Tensor))
                else w1_orig
            )
            w2_f8, w2_s = _convert_to_fp8_with_scales(
                w2_orig[expert_id]
                if isinstance(expert_id, (int, torch.Tensor))
                else w2_orig
            )
    ...
    if not use_serialized_mode:
        ...
        for i, expert_idx in enumerate(unique_ids_list):
            ...
            if fp8_enabled:
                ...
                t_s = torch.ones(
                    (count, tokens.shape[1] // 64), device="cuda", dtype=torch.float32
                ) * (tokens.abs().max() / 448.0)
                ...
                res_s = torch.ones(
                    (count, res.shape[1] // 64), device="cuda", dtype=torch.float32
                ) * (res.abs().max() / 448.0)
