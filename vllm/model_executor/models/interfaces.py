...
def supports_pp(model: nn.Module) -> bool:
    return isinstance(model, SupportsPP)


def supports_mrope(model: nn.Module) -> bool:
    return isinstance(model, SupportsMRoPE)


def supports_xdrope(model: nn.Module) -> bool:
    return hasattr(model, "get_xdrope_input_positions")


def has_inner_state(model: nn.Module) -> bool:
