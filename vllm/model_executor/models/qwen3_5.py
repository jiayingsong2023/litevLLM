...
Corrected expert and gate naming for GGUF compatibility.
"""
import torch
...
class CachedMoEExperts(nn.Module):
    def __init__(self, config, quant_config, prefix):
        ...
        # Qwen3.5 GGUF names: ffn_gate_exps, ffn_up_exps, ffn_down_exps
        # We model them as LiteLinear to use Global LRU Cache
...
class Qwen3_5MoE(nn.Module):
    def __init__(self, config, quant_config=None, prefix=""):
        super().__init__()
        self.config = config
        # GGUF name for Qwen3.5 gate: ffn_gate_inp.weight
        self.gate = LiteLinear(config.hidden_size, config.num_experts, bias=False, 
                               quant_config=None, prefix=f"{prefix}.ffn_gate_inp")
...
class Qwen3_5Layer(nn.Module):
    def __init__(self, config, layer_id, quant_config=None, prefix=""):
        ...
        if hasattr(text_config, "num_experts") and text_config.num_experts > 0:
            ...
            # Qwen3.5 Dense uses standard SwiGLU but names it gate_up_proj usually. 
            # We map it to Qwen3_5MLP
