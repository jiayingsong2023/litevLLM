...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(self, x, positions, kv_cache, attn_metadata, lora_mapping=None):
        ...
        return self._standard_forward(
            x, positions, kv_cache, attn_metadata, lora_mapping
        )

    def _standard_forward(
        self, x, positions, kv_cache, attn_metadata, lora_mapping=None
    ):
        h = self.input_layernorm(x)
        q = self.self_attn.q_proj(h, lora_mapping)
        k = self.self_attn.k_proj(h, lora_mapping)
        v = self.self_attn.v_proj(h, lora_mapping)
        bs, seq = q.shape[:2]
        q = q.view(bs, seq, self.self_attn.num_heads, self.self_attn.head_dim)
        k = k.view(bs, seq, self.self_attn.num_kv_heads, self.self_attn.head_dim)
        v = v.view(bs, seq, self.self_attn.num_kv_heads, self.self_attn.head_dim)
        ...
        if slot_mapping is not None and kv_cache is not None:
            ...
            kv_cache_dtype = (
                ...
                if inf_config
                    ...
                    attn_metadata.get("kv_cache_dtype", "auto")
                    if isinstance(attn_metadata, dict)
                    else getattr(attn_metadata, "kv_cache_dtype", "auto")
                )
            )
            k_scale = (
                inf_config.k_scale
                if inf_config
                    ...
                    attn_metadata.get("k_scale", 1.0)
                    if isinstance(attn_metadata, dict)
                    else getattr(attn_metadata, "k_scale", 1.0)
                )
            )
            v_scale = (
                inf_config.v_scale
                if inf_config
                    ...
                    attn_metadata.get("v_scale", 1.0)
                    if isinstance(attn_metadata, dict)
                    else getattr(attn_metadata, "v_scale", 1.0)
                )
            )
            ...
            reshape_and_cache(
                k.reshape(
                    -1, self.self_attn.num_kv_heads, self.self_attn.head_dim
                ).contiguous(),
                v.reshape(
                    -1, self.self_attn.num_kv_heads, self.self_attn.head_dim
                ).contiguous(),
            ...
            attn_in = torch.empty(
                (bs * seq, self.self_attn.num_heads, self.self_attn.head_dim),
                device=q.device,
                dtype=q.dtype,
            )
            block_tables = (
                attn_metadata.get("block_tables", None)
                if isinstance(attn_metadata, dict)
                else getattr(attn_metadata, "block_tables", None)
            )
            seq_lens = (
                attn_metadata.get("seq_lens", None)
                if isinstance(attn_metadata, dict)
                else getattr(attn_metadata, "seq_lens", None)
            )
            is_prefill = (
                attn_metadata.get("is_prefill", False)
                if isinstance(attn_metadata, dict)
                else getattr(attn_metadata, "is_prefill", False)
            )
...
class LlamaModel(nn.Module):
    def __init__(self, hf_config, quant_config, prefix="model"):
        ...
        self.layers = nn.ModuleList(
            [
                LlamaDecoderLayer(self.config, quant_config, f"layers.{i}", layer_idx=i)
...
class LlamaForCausalLM(nn.Module):
    def __init__(self, vllm_config, prefix=""):
        ...
        self.model = LlamaModel(
            vllm_config.model_config.hf_config, vllm_config.quant_config, "model"
        )
