...
class TritonAttention(nn.Module):
    ...
    def __init__(
        self, config: Any, quant_config: Optional[Any] = None, prefix: str = ""
    ):
        super().__init__()
        self.config = config
    ...
    def forward(self, hidden_states, positions, kv_cache, attn_metadata):
        ...
        kv_cache_dtype = (
            ...
            if inf_config is not None
                ...
                attn_metadata.get("kv_cache_dtype", "turbo_int4")
                if isinstance(attn_metadata, dict)
                else getattr(attn_metadata, "kv_cache_dtype", "turbo_int4")
            )
        )
        k_scale = (
            inf_config.k_scale
