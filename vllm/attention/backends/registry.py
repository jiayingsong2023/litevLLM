...
class AttentionBackendEnum(Enum, metaclass=_AttentionBackendEnumMeta):
    ...
    FLASH_ATTN = "vllm.attention.backends.flash_attn.FlashAttentionBackend"
    FLASH_ATTN_DIFFKV = (
        "vllm.attention.backends.flash_attn_diffkv.FlashAttentionDiffKVBackend"
    )
    TRITON_ATTN = "vllm.attention.backends.triton_attn.TritonAttentionBackend"
    ROCM_ATTN = "vllm.attention.backends.rocm_attn.RocmAttentionBackend"
    ROCM_AITER_MLA = "vllm.attention.backends.mla.rocm_aiter_mla.AiterMLABackend"
    ROCM_AITER_TRITON_MLA = (
        "vllm.attention.backends.mla.aiter_triton_mla.AiterTritonMLABackend"
    )
    ROCM_AITER_FA = (
        "vllm.attention.backends.rocm_aiter_fa.AiterFlashAttentionBackend"
    )
    ROCM_AITER_MLA_SPARSE = (
        "vllm.attention.backends.mla.rocm_aiter_mla_sparse.ROCMAiterMLASparseBackend"
    )
    TORCH_SDPA = ""  # this tag is only used for ViT
    FLASHINFER = "vllm.attention.backends.flashinfer.FlashInferBackend"
    FLASHINFER_MLA = (
        "vllm.attention.backends.mla.flashinfer_mla.FlashInferMLABackend"
    )
    TRITON_MLA = "vllm.attention.backends.mla.triton_mla.TritonMLABackend"
    CUTLASS_MLA = "vllm.attention.backends.mla.cutlass_mla.CutlassMLABackend"
    FLASHMLA = "vllm.attention.backends.mla.flashmla.FlashMLABackend"
    FLASHMLA_SPARSE = (
        "vllm.attention.backends.mla.flashmla_sparse.FlashMLASparseBackend"
    )
    FLASH_ATTN_MLA = "vllm.attention.backends.mla.flashattn_mla.FlashAttnMLABackend"
    IPEX = "vllm.attention.backends.ipex.IpexAttentionBackend"
    ...
    TREE_ATTN = "vllm.attention.backends.tree_attn.TreeAttentionBackend"
    ROCM_AITER_UNIFIED_ATTN = (
        "vllm.attention.backends.rocm_aiter_unified_attn."
...
class MambaAttentionBackendEnum(Enum, metaclass=_AttentionBackendEnumMeta):
    ...
    MAMBA1 = "vllm.attention.backends.mamba1_attn.Mamba1AttentionBackend"
    MAMBA2 = "vllm.attention.backends.mamba2_attn.Mamba2AttentionBackend"
    SHORT_CONV = "vllm.attention.backends.short_conv_attn.ShortConvAttentionBackend"
    LINEAR = "vllm.attention.backends.linear_attn.LinearAttentionBackend"
    GDN_ATTN = "vllm.attention.backends.gdn_attn.GDNAttentionBackend"
    # Placeholder for third-party/custom backends - must be registered before use
    # set to None to avoid alias with other backend, whose value is an empty string
    CUSTOM = None
